...
export const WeeklyCalendar: React.FC<WeeklyCalendarProps> = ({ schedule, isCompact = false }) => {
                    ...
                    <div key={hour} className={`${isCompact ? 'h-12' : 'h-16'} border-b border-gray-200 relative hover:bg-blue-50 transition-colors`}>
                      ...
                      {getClassesForDayAndHour(day, hour).map((classInfo, index) => {
                        ...
                        return (
                          ...
                          >
                            <div className={`font-semibold ${isCompact ? 'text-xs' : 'text-sm'} leading-tight mb-1`}>{classInfo.subject.code}</div>
                            {!isCompact && <div className="text-xs opacity-90 leading-tight mb-1">{classInfo.subject.name.length > 20 ? classInfo.subject.name.substring(0, 20) + '...' : classInfo.subject.name}</div>}
