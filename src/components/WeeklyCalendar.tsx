      ...
      <div className="overflow-x-auto">
        <div className="inline-flex min-w-full">
          ...
          <div className="flex-shrink-0 sticky left-0 z-20 bg-white border-r-2 border-gray-300">
            <div
              className={`${
                isCompact ? "p-2 h-8" : "p-3 h-12"
              } font-semibold text-gray-800 text-center bg-gradient-to-r from-blue-100 to-indigo-100 border-b-2 border-blue-200`}
            >
              🕐 Hora
            </div>
            <div className="bg-gradient-to-b from-gray-100 to-gray-50">
              {hours.map((hour) => (
                <div
                  ...
                  } text-center text-gray-700 border-b border-gray-200 flex items-center justify-center bg-gradient-to-r from-gray-50 to-gray-100 hover:from-blue-50 hover:to-blue-100 transition-colors`}
                >
                  <span
                    className={`${
                ...
                <div className="relative">
                  {hours.map((hour) => (
                    <div
                      ...
                      className={`${
                        isCompact ? "h-12" : "h-16"
                      } border-b border-gray-200 relative hover:bg-blue-50 transition-colors`}
                    >
                      {/* Render classes that start in this hour */}
                      {getClassesForDayAndHour(day, hour).map(
                        (classInfo, index) => {
                          const duration =
                            timeToMinutes(classInfo.slot.endTime) -
                            timeToMinutes(classInfo.slot.startTime);
                          const heightInRem =
                            (duration / 60) * (isCompact ? 3 : 4);
                          const startMinute = parseInt(
                            classInfo.slot.startTime.split(":")[1]
                          );
                          const topOffset =
                            (startMinute / 60) * (isCompact ? 3 : 4);
                          ...
                          return (
                            ...
                            >
                              ...
                              <div
                                ...
                                } opacity-90 font-medium bg-black bg-opacity-20 rounded px-1`}
                              >
                                {generator.formatTimeToAMPM(
                                  classInfo.slot.startTime
                                )}{" "}
                                -{" "}
                                {generator.formatTimeToAMPM(
                                  classInfo.slot.endTime
                                )}
                              </div>
                            </div>
                          );
